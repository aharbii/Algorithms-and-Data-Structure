...
void heap_create(MaxHeap *h, int *array, int size)
{
    /* O(nlog(n)) */
    h->size = 0;
    h->n = size;
...
void heap_sort(MaxHeap *h, int *array, int size)
{
...
void heap_sort_display(MaxHeap h)
{
    for (int i = 0; i < h.n; i++)
    {
        printf("%d ", h.data[i]);
...
void heapify(MaxHeap *h, int *array, int size)
{
    /* O(n) */
    h->size = size;
    h->n = size;
